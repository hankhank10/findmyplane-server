...
def request_new_plane_instance ():
    ...
    data_to_send = {
        'title': "Random Plane Title",
        'atc_id': "Random ATC ID",
    }
...
def update_location():
    ...
    # Get data from sim
    try:
        current_latitude = random.randint(-89,89)
        current_longitude = random.randint(-179,179)
        current_altitude = random.randint(0,35000)
        current_compass = random.randint(1,365)
    except:
...
print ("Attempting to connect to fake simulator...")
try:
    pass
except:
    print ("... no sim found")
...
try:
    while run_forever:
        update_location()
        input ("Press a key to send another datapoint")
